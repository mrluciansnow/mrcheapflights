...
export async function onRequestPost(context) {
  ...
  const runId = await startOp(context.env, 'scrape');
  ...
  try {
    ...
    await finishOp(context.env, runId, 'scrape', summary.errors.length === 0, summary);
    return Response.json(summary);
  } catch (err) {
    await finishOp(context.env, runId, 'scrape', false, { error: err.message });
